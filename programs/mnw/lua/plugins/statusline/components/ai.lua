local mode_helpers = require("plugins.statusline.components.mode")
...
local Copilot = {
	condition = function()
		...
		return ok
			and not copilot_client.is_disabled()
			and copilot_client.buf_is_attached(vim.api.nvim_get_current_buf())
	end,
	update = {
		"User",
		...
		if args.match == "CodeCompanionRequestStarted" then
			...
			codecompanion_timer:start(
				...
				vim.schedule_wrap(function()
					vim.api.nvim_exec_autocmds("User", { pattern = "CodeCompanionUpdated" })
				end)
			)
...
local function get_codecompanion_icons()
	return codecompanion_processing and get_spinner() .. " " or " "
end

local CodeCompanion = {
	provider = function()
		return get_codecompanion_icons() .. " "
	end,
	udpate = {
		"User",
