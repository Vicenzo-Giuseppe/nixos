local lsp_icons = require("plugins.statusline.lsp_icons")
...
return {
	...
	event = vim.g.distribution == "lazyvim" and "LazyFile" or { "BufReadPost", "BufNewFile", "BufWritePre" },
	dependencies = {
		{
			...
			opts = {
				...
				client_format = function(client_name, spinner, series_messages)
					local not_done = vim.tbl_contains(series_messages, function(message)
						return not message.done
					end, { predicate = true })
					return {
						name = client_name,
				...
				format = function(client_messages)
					...
					local clients = vim.tbl_filter(function(client)
						return vim.lsp.buf_is_attached(0, client.id)
	...
	config = function()
		require("heirline").setup({
			...
			opts = {
				colors = require("plugins.statusline.colors").colors,
			},
		})
		...
		vim.api.nvim_create_autocmd("ColorScheme", {
			callback = function()
				require("statusline.utils").on_colorscheme(require("plugins.statusline.colors").setup_colors())
			end,
			group = "Heirline",
		})
	end,
