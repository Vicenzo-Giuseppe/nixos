...
in
  pkgName: let
    hmModule =
      if inputs ? ${pkgName}
      then inputs.${pkgName}.homeManagerModules.default or null
      else null;
    ...
    sopsModule =
      if inputs ? sops
      then inputs.sops.homeManagerModules.sops or null
      else null;
    ...
    progModule =
      if progModuleResult.success
      then progModuleResult.value
      else
        import (progDir + "/${pkgName}/default.nix") {
          inherit lib pkgs;
          enabled = true;
    ...
    progConfig =
      if hmConfig != null
      then hmConfig.config.programs.${pkgName} or {}
      else {};
    ...
    flakePkg =
      if hasFlakeInput
      then inputs.${pkgName}.packages.${currentSystem}.default
      else null;
    ...
    warnMsg =
      if hmConfig == null
      then "Warning: ${pkgName} HM config failed - using fallback"
      else if !hasEnvsFile && hasSopsModule
      then "Warning: ${pkgName} envs.yaml not found - add programs/sops/envs.yaml for secrets"
      else if hasEnvsFile
      then "Warning: ${pkgName} using envs.yaml (unencrypted)"
      else if !hasSopsModule
      then "Warning: ${pkgName} no sops module - secrets not available"
      else if !hasExternalHMModule
      then "Warning: ${pkgName} has no homeManagerModules - using local config"
      else null;
  in
